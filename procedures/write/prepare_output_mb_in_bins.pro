...
if write_mb_elevationbands eq 'y' then begin
   ...
   if c[0] eq '' then begin
      spawn,'mkdir '+dirres+'/'+time_resolution+'/'+dir_region+b+'/mb_elevation' & spawn,'chmod a+rx '+dirres+dir_region+b+'/mb_elevation'
   endif

   openw,8,dirres+'/'+time_resolution+'/'+dir_region+b+'/mb_elevation/belev_'+id[gg[g]]+'.dat'
   ...
   c=findfile(dirres+'/'+time_resolution+'/'+dir_region+b+'/refr_elevation')
   if c[0] eq '' then begin
      spawn,'mkdir '+dirres+'/'+time_resolution+'/'+dir_region+b+'/refr_elevation' & spawn,'chmod a+rx '+dirres+dir_region+b+'/refr_elevation'
   endif
   openw,40,dirres+'/'+time_resolution+'/'+dir_region+b+'/refr_elevation/refrelev_'+id[gg[g]]+'.dat'
   a='' & for i=0,years-1 do a=a+string(i+tran[0],fo='(i4)')+'  '
   printf,40,'Elev  '+a &  elev_refr=dblarr(years,nb)+snoval 

   if debris_supraglacial eq 'y' then begin
   ; elevation-specified debris files 
      c=findfile(dirres+'/'+time_resolution+'/'+dir_region+b+'/debris_elevation')
      if c[0] eq '' then begin
         spawn,'mkdir '+dirres+'/'+time_resolution+'/'+dir_region+b+'/debris_elevation' & spawn,'chmod a+rx '+dirres+dir_region+b+'/debris_elevation'
      endif
      openw,41,dirres+'/'+time_resolution+'/'+dir_region+b+'/debris_elevation/debthick_'+id[gg[g]]+'.dat'
      a='' & for i=0,years-1 do a=a+string(i+tran[0],fo='(i4)')+'  '
      printf,41,'Elev  '+a &  elev_debthick=dblarr(years,nb)+snoval 

      openw,42,dirres+'/'+time_resolution+'/'+dir_region+b+'/debris_elevation/debfrac_'+id[gg[g]]+'.dat'
      printf,42,'Elev  '+a &  elev_debfrac=dblarr(years,nb)+snoval 

      openw,43,dirres+'/'+time_resolution+'/'+dir_region+b+'/debris_elevation/debfactor_'+id[gg[g]]+'.dat'
      printf,43,'Elev  '+a &  elev_debfactor=dblarr(years,nb)+snoval 

      openw,44,dirres+'/'+time_resolution+'/'+dir_region+b+'/debris_elevation/pondarea_'+id[gg[g]]+'.dat'
      printf,44,'Elev  '+a &  elev_pondarea=dblarr(years,nb)+snoval 

      if eval_mbelevsensitivity eq 'y' then begin
         openw,44,dirres+'/'+time_resolution+'/'+dir_region+b+'/debris_elevation/mbsensitivity_'+id[gg[g]]+'.dat'
