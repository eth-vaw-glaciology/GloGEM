...
if firnice_write[0] eq 'y' then begin
    ...
    openw,46, firnice_dir + '/temp_10m_'     + id[gg[g]] + '.dat'
    printf,46,'Elev  '+a

    openw,47, firnice_dir + '/temp_50m_'     + id[gg[g]] + '.dat'
    printf,47,'Elev  '+a

    openw,48, firnice_dir + '/temp_bedrock_' + id[gg[g]] + '.dat'
    printf,48,'Elev  '+a
endif

if enable_advection eq 'y' AND advection_write eq 'y' then begin
    openw,70, firnice_dir + '/adv_horizontal_' + id[gg[g]] + '.dat'
    a='' & FOR i=0,years-1 DO a=a+string(i+tran[0],fo='(i4)')+'  '
    printf,70,'Elev  '+a
    elev_adv_horiz=dblarr(years,nb)+snoval

    openw,71, firnice_dir + '/adv_vertical_' + id[gg[g]] + '.dat'
    printf,71,'Elev  '+a
...
if firnice_write[1] eq 'y' then begin
    ...
    for j=0,n_elements(firnice_profile)-1 do begin
        openw,51+j, firnice_dir + '/temp_ID' + firnice_profile_ID[j] + '_' + id[gg[g]] + '.dat'
