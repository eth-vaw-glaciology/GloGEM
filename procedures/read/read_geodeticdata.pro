...
; read_geodeticdata
;
; Read glacier-specific geodetic mass balance data for the
; calibration period from the Hugonnet et al. regional dataset.
;
; Constructs the file path from the active region and calibration
; period, reads the glacier-level mass balance estimates (calimb_bn)
; with associated area and uncertainty flags, and filters outliers
; by replacing values beyond two standard deviations for small
; glaciers (area below 20 km2) with the area-weighted regional mean.
; The cleaned array is used as the calibration target for individual-
; glacier parameter optimisation.
; *************************************************************
