...
if single_glacier ne '' then begin
    subpath='/files'+plf+'/files_original/SINGLE/'
endif else if reanalysis_direct eq 'y' then begin
    subpath='/PAST'+version_past+mtt+'/PAST_original/'
endif else begin
    subpath='/files'+plf+'/files_original/'+GCM_model[gcms]+'/'+GCM_rcp[rcps]+'/'
endelse

if past_out eq 'y' and hindcast_dynamic eq 'y' and reanalysis_direct eq 'y' then subpath='/PAST'+version_past+mtt+'/PAST_original/dyn/'
