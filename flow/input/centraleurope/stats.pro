...
data_rgi = import_rgi_data(script_path + '/rgi/11_rgi60_CentralEurope.csv')
restore, script_path + '/inventory_date.sav'

; set file path
filename = script_path + string(glacier_id, format = '("/flowline_geom/",I05,".dat")')
...
for glacier_id = 1, 3927 do begin ; Range over which geometry files exist
  ...
  data_glaciers[glacier_id - 1, 0] = glacier_geom[length_array - 1, 6] / 1000.0 ; Glacier length (km)
  data_glaciers[glacier_id - 1, 1] = total(glacier_geom[*, 3]) ; Glacier area (km^2)

  vol = 0.0
  for i = 0, length_array - 1 do begin
    vol = vol + glacier_geom[i, 3] * glacier_geom[i, 4] / 1000.0 ; area*thick
  endfor

  data_glaciers[glacier_id - 1, 2] = vol ; Glacier volume (km^3)
endfor

; Create scatter plot for glacier length
w1 = window(dimensions = [800, 600])
s1 = scatterplot(indgen(3927) + 1, data_glaciers[*, 0], /current, $
  symbol = 'circle', sym_filled = 1, $
  xtitle = 'Glacier ID', ytitle = 'Length (km)', $
  font_size = 18)

; Create scatter plot for glacier area
w2 = window(dimensions = [800, 600])
s2 = scatterplot(indgen(3927) + 1, data_glaciers[*, 1], /current, $
  symbol = 'circle', sym_filled = 1, $
  xtitle = 'Glacier ID', ytitle = 'Area (km²)', $
  font_size = 18)

; Create scatter plot for glacier volume
w3 = window(dimensions = [800, 600])
s3 = scatterplot(indgen(3927) + 1, data_glaciers[*, 2], /current, $
  symbol = 'circle', sym_filled = 1, $
  xtitle = 'Glacier ID', ytitle = 'Volume (km³)', $
  font_size = 18)
...
; Save variables to a .sav file in script_path
save, index_larger_than_1_km_glaciers_save, $
  ...
  index_smaller_than_1_km2_glaciers_but_longer_than_1_km_save, $
  filename = script_path + '/glacier_stats.sav'
