...
; Define default parameters
  ...
  ENDIF ELSE BEGIN
    ...
    FOR i=0, N_ELEMENTS(tags)-1 DO BEGIN
      tag = tags[i]
      IF ~TAG_EXIST(params, tag) THEN BEGIN
...
WHILE time LT start_year + params.nyears DO BEGIN
  ...
  IF (time - next_time_diag) GE 0 THEN BEGIN
     diagnostic_write
     breakindex, vol = instability_iceleavedomain_steadystate(glacier_id, th, params.smb_sinus_flag, vol_hist, counter_diag, params.ss_criterion, time, start_year, glacier_length, domain_exit_index, vol, obs_vol_flowlinemodel, params.mb_type_flag)
