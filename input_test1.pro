...
dirres = '/scratch_net/vierzack05/mvantiel/GloGEM_output/r' + RGIversion + '_' + time_resolution + '/' ; output folder  (same machine as you run on)scratch via the network
...
for i = 0, fit_layers[0] - 1 do fit_dz[0, i] = fit_dzstep[0]
for i = fit_layers[0], total(fit_layers[0 : 1]) - 1 do fit_dz[0, i] = fit_dzstep[1]
for i = total(fit_layers[0 : 1]), total(fit_layers[0 : 2]) - 1 do fit_dz[0, i] = fit_dzstep[2]
