...
if calibrate eq 'n' and tran(1) lt 2023 then begin
   reanalysis_direct='y'
   short_gcmchoice=[1,1,1]   ; make sure that only one run!
   glacier_retreat='n'
...
if tran(1) gt 2022 then hindcast_dynamic='n'
...
if firnice_temperature eq 'y' then begin
   READ_GEOTHERMAL,dir,firnice_geotherm_flux
   if firnice_batch eq 'y' then READ_FIRNICEBATCH,dir,firnice_batch_data1,firnice_batch_data2
endif
...
if firnice_temperature eq 'y' then begin
   ...
   FIRNICE_TEMPERATURE_MODEL,gl,fit_layers,fit_dens,fit_dz, rf_dsc, tgs,tl_fit,te_fit,geothermal_flux, cair,cice,kair,kice, sno,mel,thick,slope,firn, firnice_batch,firnice_write,firnice_maxdepth, fit_water,fact_permeability,elev_firnicetemp,firnice_profile,ye,tran,m
...
if ar_gl ne 0 then begin
   ...
   if bal_month eq 274 then if m eq 121 then wb(ye)=total(bal*area)/ar_gl
