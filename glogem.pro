...
fn = 'input.pro'
anz = file_lines(fn)
input_file_content = strarr(anz)
openr, 1, fn
readf, 1, input_file_content
close, 1
        ...
        for re = 0, region_id_loop[1] - region_id_loop[0] do begin
          ...
          if calibrate eq 'y' then begin
            if calibrate_glacierspecific eq 'n' then begin
              fn = dir + 'calibration.dat'
              anz = file_lines(fn) - 1
              openr, 1, fn
              readf, 1, s
              readf, 1, tt
              close, 1
              calimb_regname = strarr(anz)
              calimb_sregname = strarr(anz)
              calimb_outline = strarr(anz)
              calimb_idname = dblarr(anz)
              calimb_p0 = dblarr(anz)
              calimb_p1 = dblarr(anz)
              calimb_bn = dblarr(anz)
              calimb_uc = dblarr(anz)
              for i = 0l, anz - 1 do begin
                a = strsplit(tt[i], ' ', /extract)
                calimb_regname[i] = a[0]
                calimb_sregname[i] = a[1]
                calimb_outline[i] = a[2]
                calimb_idname[i] = double(a[4])
                calimb_p0[i] = double(a[5])
                calimb_p1[i] = double(a[6])
                calimb_bn[i] = double(a[7])
                calimb_uc[i] = double(a[8])
              endfor
          ...
          for cphl = 1, double(caliphase_loop) do begin
            ...
            if calibrate eq 'y' then begin
              if calibrate_glacierspecific eq 'n' then begin
                ...
                target = calimb_bn[ii[0]]
                target_uc = calimb_uc[ii[0]]
                cran = [calimb_p0[ii[0]], calimb_p1[ii[0]]]
            ...
            if read_parameters eq 'y' then begin
              ...
              a = findfile(fn)
              if a[0] eq '' then print, '!!! Parameter-File for ' + sub_region + ' is not available !!!'
              ...
              ii = where(da[cnc - 1, *] eq 1, ci)
              jj = where(da[cnc - 1, *] eq 0, cj)
            ...
            DDFsnow = double(DDFsnow)
            DDFice = double(DDFice)
            C0 = double(C0)
            C1 = double(C1)
            ...
            fn = dir_data + '../files/thick_' + region + '.dat'
            anz = file_lines(fn) - 1
            s = strarr(1)
            st = strarr(anz)
            openr, 1, fn
            ...
            if valiglaciers_only eq 'y' then begin
              fn = dir + validation_dataset + dir_region + '.dat'
              an = file_lines(fn) - 1
              ss = strarr(2, an)
              for i = 0l, anz - 1 do begin
                a = double(id[i]) - double(ss[1, *])
                if min(abs(a)) ne 0 then a_gl[i] = -1. ; setting area to negative, so that it will not be computed
              endfor
            endif
            ...
            if c[0] eq '' then begin
              spawn, 'mkdir ' + dirres + dir_region
              spawn, 'chmod a+rx ' + dirres + dir_region
              spawn, 'mkdir ' + dirres + dir_region + '/calibration'
              spawn, 'chmod a+rx ' + dirres + dir_region + '/calibration' ; calibration folder
              spawn, 'mkdir ' + dirres + dir_region + '/files' + mtt
              spawn, 'chmod a+rx ' + dirres + dir_region + '/files' + mtt ; result files
              spawn, 'mkdir ' + dirres + dir_region + '/PAST' + mtt
              spawn, 'chmod a+rx ' + dirres + dir_region + '/PAST' + mtt ; past files
            endif

            c = findfile(dirres + dir_region + '/files/SINGLE')
            if c[0] eq '' then begin
              spawn, 'mkdir ' + dirres + dir_region + '/files/SINGLE'
              spawn, 'chmod a+rx ' + dirres + dir_region + '/files/SINGLE'
            endif

            c = findfile(dirres + dir_region + '/files' + mtt + '/' + GCM_model[gcms])
            if c[0] eq '' then begin
              spawn, 'mkdir ' + dirres + dir_region + '/files' + mtt + '/' + GCM_model[gcms]
              spawn, 'chmod a+rx ' + dirres + dir_region + '/files' + mtt + '/' + GCM_model[gcms]
            endif
            ...
            if catchment_selection ne '' then begin
              ...
              fn = dir + 'catchments/RGI' + a + '_' + catchment_selection + '.dat'
              an = file_lines(fn) - 1
              s = strarr(an)
              tt = strarr(1)
              openr, 1, fn
              ...
              for i = 0l, n - 1 do begin
                ii = where(id[i] eq ss, ci)
                if ci gt 0 then tt[i] = 1
              endfor
              ii = where(tt eq 1, ci)
              if ci gt 0 then begin
                hmed = hmed[ii]
                hmin = hmin[ii]
                survey_year = survey_year[ii]
                volume_ini = volume_ini[ii]
                xy = xy[*, ii]
                a_gl = a_gl[ii]
                id = id[ii]
              endif
                ...
                for gy = 0, ngy - 1 do begin
                  ...
                  for g = 0l, cg - 1 do begin
                    ...
                    for cal1 = 0, cal1max do begin
                      ...
                      fn = dir_data + '/' + region + '/' + id[gg[g]] + '.dat'
                      a = findfile(fn)
                      ...
                      if a[0] ne '' then begin
                        ...
                        if firnice_temperature eq 'y' then begin
                          a = min(abs(latitudes[g] - fit_yy), indy)
                          a = min(abs(longitudes[g] - fit_xx), indx)
                        ...
                        width = da[5, *]
                        slope = da[7, *]
                        ii = where(area gt 0 and thick eq 0, ci)
                        if ci gt 0 then thick[ii] = 3. ; prevent division by 0 due to error in thick-file
                        bed_elev = elev - thick
                        step = elev[1] - elev[0]
                        e0 = elev[0]
                        elev0 = elev
                        ...
                        if calibrate eq 'y' and ci eq 0 then thick = thick + 1.
                        gl = dblarr(nb) + noval
                        if ci gt 0 then gl[ii] = elev[ii]
                        length = dblarr(nb)
                        for i = 0, nb - 1 do length[i] = (max(da[6, *]) - da[6, i]) / 1000.
                        thick_ini = thick
                        area_ini = area
                        ...
                        if write_mb_elevationbands eq 'y' then begin
                          ...
                          if c[0] eq '' then begin
                            spawn, 'mkdir ' + dirres + dir_region + b + '/mb_elevation'
                            spawn, 'chmod a+rx ' + dirres + dir_region + b + '/mb_elevation'
                          endif

                          openw, 8, dirres + dir_region + b + '/mb_elevation/belev_' + id[gg[g]] + '.dat'
                          a = ''
                          for i = 0, years - 1 do a = a + string(i + tran[0], fo = '(i4)') + '  '
                          printf, 8, 'Elev  ' + a + a
                        ...
                        if read_parameters eq 'y' and cal1 eq 0 then begin
                          ...
                          case meltmodel of
                            '1': begin
                              DDFice = cali_ddfice[ind]
                              DDFsnow = cali_ddfsnow[ind]
                              c_prec = cali_cprec[ind]
                              t_offset = cali_toff[ind]
                            end
                            '3': begin
                              C0 = cali_c0[ind]
                              C1 = cali_c1[ind]
                              alb_ice = cali_a_ice[ind]
                              alb_snow = cali_a_snow[ind]
                              c_prec = cali_cprec[ind]
                              t_offset = cali_toff[ind]
                            end
                          endcase
                        ...
                        snostor = sur
                        firn = sur
                        ff = where(elev gt hmed[gg[g]], ci)
                        if ci gt 0 then firn[ff] = 1
                        ...
                        if time_resolution eq 'daily' then begin
                          if outf_names[14] ne '' then begin
                            accday = dblarr(years * 365.) + snoval
                            rainday = accday
                            snowmeltday = accday
                            refrday = accday
                            discharge_gl = accday
                        ...
                        for ye = 0, years - 1 do begin
                          ...
                          if ci gt 0 then ar_gl = total(area[ii]) else ar_gl = 0
                          ...
                          for d = 0, 1 do begin
                            ...
                            for m = st, en do begin
                              ...
                              jj = where(sno gt 0, cj)
                              if cj gt 0 then sur[jj] = 1
                              jj = where(sno lt 0, cj)
                              ...
                              difarea = area_iniconst - area
                              ii = where(difarea lt 0, ci)
                              if ci gt 0 then difarea[ii] = 0
                              ii = where(area_iniconst gt 0, ci)
                              dd = 0
                              ...
                              if cj gt 0 then sur[jj] = 0
                              jj = where(sno eq 0 and gl eq noval, cj)
                              if cj gt 0 then sur[jj] = noval
                              jj = where(sno eq 0 and firn eq 1, cj)
                              if cj gt 0 then sur[jj] = 2
                              ...
                              if time_resolution eq 'monthly' then begin
                              balmo[ccmon - 1] = total((psg - mel + refr) * area) / ar_gl
                              melmo[ccmon - 1] = total(mel * area) / ar_gl
                              accmo[ccmon - 1] = total(psg * area) / ar_gl
                              refrmo[ccmon - 1] = total(refr * area) / ar_gl
                              precmo[ccmon - 1] = total((psg + plg) * area) / ar_gl
                              endif else begin
                              ; for entire catchment
                              accday[ccmon - 1] = total((psg) * area_ini) / total(area_ini)
                              refrday[ccmon - 1] = total(refr * area_ini) / total(area_ini)
                              rainday[ccmon - 1] = total((plg) * area_ini) / total(area_ini)
                              snowmeltday[ccmon - 1] = total((snowmel) * area_ini) / total(area_ini)
                              icemeltday[ccmon - 1] = total((icemel) * area) / total(area_ini)
                              ...
                              ii = where(gl eq noval, ci)
                              if ci gt 0 then bal[ii] = snoval
                          ...
                          if time_resolution eq 'monthly' and eval_mbelevsensitivity eq 'y' then begin
                            if count_mbelevsens ge 0 then begin
                              ...
                              bb = n_elements(elev_mbsensall[*, 0, 0])
                              a = dblarr(1, bb)
                              b = dblarr(bb)
                              c = b
                              for i = 0, nb - 1 do begin
                              for j = 0, bb - 1 do a[0, j] = elev_mbsensall[j, ye, i]
                              for j = 0, bb - 1 do b[j] = j * 50 * (-1.)
                              for j = 0, bb - 1 do c[j] = 1. ; c not needed for IDL!
                          ...
                          if ye gt 4 then begin
                            ...
                            firn = dblarr(nb)
                            ii = where(balm gt 0 and gl ne noval, ci)
                            if ci gt 0 then firn[ii] = 1
                          endif
                          ...
                          if cj gt 0 then begin
                            ht1 = elev[jj[0]]
                            ii = where(bal[jj] gt 0, ci)
                            if ci gt 0 then aar[ye] = total(area_stor[jj[ii]]) * 100. / area1 else aar[ye] = 0
                            btongue[ye] = min(bal[jj], ind)
                            if ci gt 0 then ela[ye] = elev[jj[ii[0]]] else ela[ye] = max(elev)
                            da = (elev[jj[ind]] - ela[ye])
                            if abs(da) gt 20 then dbdz[ye] = btongue[ye] / da else dbdz[ye] = 0.
                          endif else ht1 = max(elev)
                          hmin_g[ye] = ht1

                          jj = where(gl eq noval, cj)
                          if cj gt 0 then sur[jj] = noval
                          ...
                          if write_hypsometry_files eq 'y' then begin
                            if (ye + tran[0]) mod 10 eq 0 then begin
                              hypso_file[0, chypso, *] = elev
                              hypso_file[1, chypso, *] = area
                              hypso_file[2, chypso, *] = area * thick
                              hypso_file[3, chypso, *] = tgs_cum / (10 * 12.)
                              tgs_cum = dblarr(nb) ; set array back
                              chypso = chypso + 1
                            endif
                          endif
                          ...
                          jj = where(balv gt 0, cj)
                          if cj gt 0 then av = total(balv[jj]) else av = 0
                          dens = 0.9
                          dvol = dvol / dens
                        ...
                        if calibrate_individual eq 'y' then begin
                          ...
                          if cal1 eq 0 then begin
                            fc = 2. - max([0, mean(wb) / 4.])
                            if fc lt 1.3 then fc = 1.3
                          ...
                          if abs(target - c_mb) gt cal_crit then begin
                            ...
                            if calibration_phase ne '3' then begin
                              ...
                              if cal1 gt 0 and c_mb gt target and c_mbst lt target then begin
                              t_offset0 = t_offset - 1.
                              di = c_mbst - c_mb
                              fra = (target - c_mb) / di
                              df = t_offset0 - t_offset
                              plf = df * fra
                              t_offset = t_offset0 - plf
                              endif
                              c_mbst = c_mb
                    ...
                    if firnice_temperature eq 'y' then begin
                      if firnice_write[0] eq 'y' then begin
                        for i = 0, n_elements(elev_firnicetemp[0, 0, *]) - 1 do printf, 45, elev0[i], elev_firnicetemp[0, *, i], fo = '(i6,' + strcompress(string(years, fo = '(i3)'), /remove_all) + 'f8.3)'
                        close, 45
                        for i = 0, n_elements(elev_firnicetemp[0, 0, *]) - 1 do printf, 46, elev0[i], elev_firnicetemp[1, *, i], fo = '(i6,' + strcompress(string(years, fo = '(i3)'), /remove_all) + 'f8.3)'
                        close, 46
                        for i = 0, n_elements(elev_firnicetemp[0, 0, *]) - 1 do printf, 47, elev0[i], elev_firnicetemp[2, *, i], fo = '(i6,' + strcompress(string(years, fo = '(i3)'), /remove_all) + 'f8.3)'
                        close, 47
                        for i = 0, n_elements(elev_firnicetemp[0, 0, *]) - 1 do printf, 48, elev0[i], elev_firnicetemp[3, *, i], fo = '(i6,' + strcompress(string(years, fo = '(i3)'), /remove_all) + 'f8.3)'
                        close, 48
                      endif
            ...
            fn = dircali + dir_region + '/calibration/calibrate_m' + meltmodel + '_cID' + string(calperiod_ID, fo = '(i1)') + '_' + sub_region + cc + '.dat'
            anz = file_lines(fn) - 1
            if meltmodel eq '3' then a = 2 else a = 0
            da = dblarr(13 + a, anz)
            tt = strarr(1)
            openr, 1, fn
            readf, 1, tt
            ...
            if cphl eq 1 then begin
              ...
              ii = where(flag_eval eq 2, ci)
              ii = where(flag_eval eq 3, cj)
          ...
          if calibrate eq 'y' then begin
            ...
            readf, 1, da
            close, 1
            ii = where(da[12 + a, *] eq 0, ci)
            print, '     Not calibrated: ' + string(ci * 100 / anz, fo = '(f5.2)') + '%'

            ; evaluate statistics for calibration phases
            print, '*** Calibration phase statistics:'
            a = caliphase_statistics[0]
            c = caliphase_statistics[2]
            d = caliphase_statistics[3]
            ...
            printf, 2, '1: ' + string(a, fo = '(i3)') + '%;   2:' + string(caliphase_statistics[1], fo = '(i3)') + '%;   3:' + string(c, fo = '(i3)') + '%'
            close, 2
